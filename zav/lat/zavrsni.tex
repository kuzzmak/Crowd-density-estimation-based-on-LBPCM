...
dotičnog mjesta. Identifikacija regija s gustoćom mnoštva većom
...
kojem pristupa veći broj ljudi ili kontroliranje postojećih, npr. 
...
LBP se razvio kao poseban slučaj jedinice teksture (engl. \textit{Texture unit})
...
Lokalna informacija teksture izvlači se iz susjedstva \(3 \times 3\) koje 
predstavlja najmanju potpunu jedinicu (u smislu postojanja osam 
smjerova oko piksela). S obzirom na \(3 \times 3\) susjedstvo koje će biti 
označeno skupom od devet elemenata \(V = \left\{V_0, V_1,..., V_8\right\}\),
gdje \(V_0\) predstavlja intenzitet srednjeg piksela, a 
\(\left\{V_0, V_1,..., V_8\right\}\) predstavlja entitete susjednih piksela.
Jedinica teksture je skup od osam elemenata TU = \(\left\{E_1, E_2,..., E_8\right\}\),
gdje se \(E_i\) (i = 1,2,...,8) određuje formulom
...
\[
N_{TU} = \sum_{i=1}^{8} E_i \cdot 3^{i-1}
\]
...
LBP se naziva jednoličnim ako ima najviše dvije 0-1 ili 1-0 tranzicije.
 ...
 svrstavaju u zajedničku grupu. Koristeći jednolične uzorke,
  dimenzionalnost vektora značajki možemo smanjiti s 256 na 59 
...
Primjer računanja matrice pojavnosti sivih razina i za \(d=0, \theta=0\). Slika 
...
Prikazana GLCM je oblika \(4 \times 4\) jer sive razine izvorne slike mogu poprimiti
4 različite vrijednosti. U radu se koriste slike zapisane u 8-bitnom formatu
što znači da se veličina matrice povećava na \(256 \times 256\) jer svaki piksel može
...
koja je u suštini GLCM samo što se izračunavanje vrijednosti matrice 
...
najbližih susjeda. Ako je \(k=1\), labelu određuje samo najbliži susjed,
ako je \(k=3\), razred s više glasova pobjeđuje te se kao labela novome, dosad
neviđenome uzorku, dodjeljuje baš taj razred. Za broj susjeda se odabire pozitivan
...
sve nestabilnije, npr. ako postoji jedan primjer nekog drugog 
...
\begin{itemize}
	...
	radijusa). Pripadnost se određuje na temelju najbližeg susjeda pa labela
...
ako postoji neki skup vektora \textbf{X} u kojem svaki od vektora \(x_i, i = 0, 1, ..., N\), 
...
\(g(\boldsymbol{x}) = \omega^T\boldsymbol{x}+\omega_0=0\).
...
označene su crnim linijama na slici 2.12 (Preuzeta iz \citep{pattern}). Margina udaljenosti 1 je 2\(z_1\), a udaljenosti 2 je 2\(z_2\). 
Cilj je pronaći smjer koji daje maksimalnu moguću marginu. Margina je najmanja udaljenost 
...
Želimo postići da je vrijednost \(g(\boldsymbol{x})\) u blizini najbliže točke jednaka 1 za razred \(\omega_1\), a
...
\begin{multicols}{2}
Slučaj kada razredi nisu linearno odvojivi prikazan je na slici 2.13. Koliko god se trudili nikako 
...
linearno odvojivi. Uzmimo kao primjer sliku 2.13. i prijeđimo u trodimenzionalni sustav. Novonastala 
...
od drugih tako da ne postoje preklapanja dvaju razreda koja bi rezultirala krivom klasifikacijom.
...
Funkcija kojom se prelazi u drugu dimenziju označava se s \(\varphi(\boldsymbol{x})\). Kako bi se navedena ideja
koristila u funkciji koja se maksimizira, dovoljno je samo napraviti sljedeće
jer funkcija ovisi samo o polaznom skupu uzoraka, odnosno \textbf{X}.

\[
L = \sum \alpha_i - \frac{1}{2} \sum_i \sum_j \alpha_i \alpha_j y_i y_j \varphi(x_i) \cdot \varphi(x_j)
\]

Kraći zapis korišten za \(\varphi(x_i) \cdot \varphi(x_j)\) je \(k(x_i, x_j)\) čime se označavaju 
...
\begin{itemize}
	\item jedan protiv svih gdje se labela uzorku dodjeljuje prema klasifikatoru koji 
...
Vladimira Vapnika čija ideja u tadašnje vrijeme nije najbolje prihvaćena. Tek se 1990.-ih daje 
...
Kako je funkcija intenziteta slike poznata samo u diskretnim točkama, derivacija ove funkcije 
...
Ovaj operator koristi dvije 3 \(\times\) 3 jezgre koju su konvoluirane s izvornom slikom kako bi se aproksimirale 
derivacije – jedna za x smjer, a druga za y smjer. Označimo izvornu sliku kao dvodimenzionalnu matricu \(A\), sliku
...
GLCM te iz koje se izračunavaju i same značajke. Ako slika nije dovoljno velika, neće sadržavati
...
\(768 \times 576\), u sive razine i dijeljenje njih samih u manje podslike, dimenzija
\(192 \times 144\), kojih ima 16 po svakoj izvornoj slici. Svaka podslika razlikuje se 
...
značajki, koristeći se tehnikom kliznog okna, konstruira se matrica 
...
sastoji se od vrijednosti Haralickovih značajki njezinih okana. Po završetku 
...
klasifikatora koristi se \(70\%\) izvornih slika, a za testiranje ostatak.
...
modificirati tako da se svakome klasifikatoru dodijeli neka težina, 
...
Promijenimo iznose težina svakog klasifikatora sljedećim \(\{0.1,0.1,0.8\}\). Sada izračun
...
klasifikatora i ako nisu ispravno dodijeljene, rezultat klasifikacije može
...
GLCM pojedine slike. PIL je korišten za pretvorbu normalne slike u oblik koji 
...
koji otvara stranicu s glavnim funkcionalnostima aplikacije. U podnožju svakog
...
operatora gradijenta na odabranoj slici.
...
\(768 \times 576\) te se podijeli u 16 jednakih podslika \(192 \times 144\). Ova konkretna dimenzija
...
unesenih dimenzija moguće je vidjeti kako bi izgledale konkretne dimenzije
...
\caption{Prikaz izgleda podslike s unesenim dimenzijama}
\end{figure}

\bigbreak

Novi dio prozora prikazuje jednu izvornu sliku iz odabranog direktorija
s podjelom na manje podslike, a one su dimenzija unesenih iznad. Svaka podslika
...
sadrži 3536 podslika u sivim razinama, ako su korištene dimenzije \(192 \times 144\), 
pa je moguće prijeći u sljedeću fazu, a to je označavanje svake pojedine podslike.
...
je jedinstveno, a 2 označava pripadni razred gustoće mnoštva. Ako je 
...
podslika učitane u memoriju ili nisu. Kako bi ih učitali potrebno je pritisnuti
...
stvara više konfiguracija koje se mogu izvoditi istovremeno ako je računalo
...
odnosno podslike koje nije vidio tijekom faze učenja. Ako nije niti 
...
u \enquote{model loaded} sa zelenom kvačicom.
...
rezultat klasifikacije. Vrh stranice prikazuje paletu boja koje
...
ne neku drugu koja se brže izračunava stoga je potrebno naći nekakav
...
Opisani problem bi se manifestirao tako da su neka 
...
s već obrađenim podslikama za ilustraciju postupka. Ako je prikazan tekst 
...
prozora te pritisnuti gumb \enquote{Calculate function(s)}. Ukoliko
...
je nužan postupak jer ovdje korišteni klasifikatori koriste mjeru udaljenosti
...
dominirale i previše utjecale na rezultat klasifikacije. Postoje više različitih 
...
vrijednost te komponente i podijeli drugim korijenom iz srednje vrijednosti
...
Kada je dobiven vektor značajki jednog okna, taj se vektor doda postojećem 
vektoru značajki podslike te se postupak ponavlja za svako okno podslike.
\newline
...
Ukupna dimenzionalnost (\textit{dim}) vektora značajki pojedine podslike dobije se umnoškom:
...
LBP radijus & 1 \\
korak okna & 32 \\
veličina okna & \(64 \times 64\) \\
LBPCM udaljenosti & 1 \\
LBPCM kutovi & 0, \(\pi\) \\
\end{tabular}
\caption{Parametri primjera} 
\end{table}
...
normalizirani i raspoređeni tako da je svaki
od vektora značajki zbrojen s vektorima razreda kojem pripadaju.
...
Oznake u tablici 6.2 predstavljaju razrede gustoće:\( c_0 \rightarrow \) \enquote{no flow},
...
i svaki razredi bi sačinjavali vektori značajki koji formiraju nekakav oblik hiperkugli ili nekog srodnog oblika
...
klasifikaciju prema bližim susjedima dok u slučaju da imamo donekle pravilne grupe (hiperkugle i sl.) 
...
Veličina koraka okna od 64 i 32 piksela pokazala se kao najbolja mjera s obzirom na veličinu okna od \(64 \times 64\). 
Manji koraci ne donose nikakvu novu informaciju nego samo povećavaju vrijeme potrebno za 
obradu pojedine podslike, a koraci veći od 64 nemaju smisla jer je veličina okna zadržana
stalnom. 
...
učenja pa je stoga izbor značajki ograničeni i tom ogradom. U ovom radu nikad nije uzeto
...
\begin{minipage}{\linewidth}
\centering
\includegraphics[scale=0.6]{img/lijevo.png}
\captionof{figure}{Klasifikacije mnoštva koje se nalazi na krajnjem lijevom dijelu slike}
\end{minipage}

\bigbreak

Na slici 6.1 klasifikator nema nikakvih problema s okolnim područjem oko mnoštva koje je prazno.
...
\begin{minipage}{\linewidth}
\centering
\includegraphics[scale=0.6]{img/cijela.png}
\captionof{figure}{Klasifikacije mnoštva koje se proteže kroz cijelu sliku}
\end{minipage}

\bigbreak

Razasuto mnoštvo na slici 6.2 nije ignorirano i ispravno se klasificira.

\bigbreak

\begin{minipage}{\linewidth}
\centering
\includegraphics[scale=0.6]{img/malo.png}
\captionof{figure}{Klasifikacija mnoštva koje se nalaza na kranjem desnom dijelu slike}
\end{minipage}

\bigbreak

Na slici 6.3 moguće je vidjeti problematičnu situaciju, mnoštvo je pocijepano i rezultat nije najbolji.
...
Postupak prikazan u ovome radu prikladan je za vrstu slika kojima se parametri ne mijenjaju. Pod 
...
\begin{figure}[ht]
	\begin{subfigure}[b]{0.3\linewidth}
		\centering
		\includegraphics[scale=0.38]{img/wrong1.png}
		\caption{Pogled 1}
	\end{subfigure}
	\begin{subfigure}[b]{0.3\linewidth}
		\centering
		\includegraphics[scale=0.38]{img/wrong2.png}
		\caption{Pogled 2}
	\end{subfigure}
	\begin{subfigure}[b]{0.3\linewidth}
		\centering
		\includegraphics[scale=0.38]{img/wrong3.png}
		\caption{Pogled 3}
	\end{subfigure}
\caption{Prikaz klasifikacije drugih skupova podataka iz PETS2009}
\end{figure}

Iz priložene slike 7.1 vidi se koliko promjena izgleda slike utječe na klasifikaciju. Iako se na slici vidi 
da postoji nekakvo mnoštvo, klasifikator to nije u mogućnosti vidjeti, odnosno klasificira skoro svaku
podsliku kao da ne sadrži mnoštvo. Takvi rezultati mogu se izbjeći
...
Rezolucija svake slike iznosi \(640 \times 352\) piksela što je manje nego izvorni skup podataka 
nad kojim se prvotno vršilo eksperimentiranje. Kako bi se održala neka vrsta usporedivosti,
 slike novog skupa također su podijeljene u 16 jednakih podslika, svaka dimenzije \(160 \times 88\) piksela. 
...
veličinu okna uzeto \(40 \times 40\) piksela kako bi se pokrila cijela slika i korak okna od 20 piksela.
...
ako se postupak učenje ponovi prilikom promjene skupa podataka i ukoliko se mnoštvo te uvjeti
...
jer bi se teksturna svojstva slike drastično promijenila.

\chapter{Zaključak}

Kako je navedeno i u uvodu, procjena gustoće mnoštva izuzetno je važna komponenta sustava
za nadziranje i upravljanje mnoštvom. Video prijenos sigurnosnih kamera prilikom znatnijih 
okupljanja ljudi može se obraditi tako da prikazuju korisne informacije poput procjene
...
regulira težinom koja odgovara točnosti klasifikacije njega samog. Regulirati te težine se može i 
na druge načine, ali pritom treba pripaziti jer to uvelike može promijeniti odluke klasifikacije.

\bigbreak

Ovakva fuzija dvaju klasifikatora pokazala se dobrom što je i na kraju rezultiralo zadovoljavajućim
...
ako se problem prilagodi tako da se izračuni značajki paraleliziraju što je više moguće. 
...
značajki nad sivim slikama i sivim slikama nad kojima je primijenjen operator gradijenta. Iz takvih slika
