...
class App:
    ...
    def __init__(self):
        ...
        # rječnik konfiguracije, sadrži staze do potrebnih resursa za
        # aplikaciju
        self.configuration = {}
    ...
    def loadLabels(self):
        ...
        self.writer.loadAnnotedDataFromFile(
            os.path.join(
                self.configuration['labeledDataDirectory'],
                'labeledData.txt'))
        self.labelDictionary = self.writer.labelDictionary
        self.dataAnnotationCounter = self.writer.labelDictionary.__len__()
        self.gui.consolePrint(
            "[INFO] loaded " + str(self.labelDictionary.__len__()) + " labels")
        self.gui.frames[fvcP.FeatureVectorCreationPage].labelLabelsLoadedColor.configure(
            text="LOADED", fg="green")
    ...
    def addConf(self):
        ...
        try:
            radius = int(
                self.gui.frames[coP.ConfigurationsPage].entryLBPRadius.get())
            if radius < 1:
                raise ValueError
        except ValueError:
            parametersOK = False
        ...
        try:
            glcmDistance = [
                int(x)
                for x in
                self.gui.frames[coP.ConfigurationsPage].entryGLCMDistance.get().split(
                    ",")]
            ...
            parametersOK = False
            self.gui.consolePrint(
                "[ERROR] please check your input for glcm distance")

        try:
            stepSize = int(
                self.gui.frames[coP.ConfigurationsPage].entryStepSize.get())
        except ValueError:
            parametersOK = False
            self.gui.consolePrint(
                "[ERROR] please check your input for step size")

        try:
            cellSize = [
                int(x)
                for x in
                self.gui.frames[coP.ConfigurationsPage].entryCellSize.get().split(
                    ",")]
            if len(cellSize) != 2:
                raise ValueError
            for c in cellSize:
            ...
            parametersOK = False
            self.gui.consolePrint(
                "[ERROR] please check your input for cell size")

        try:
            angles = [
                radians(int(i))
                for i in
                self.gui.frames[coP.ConfigurationsPage].entryAngles.get().split(
                    ",")]
        except ValueError:
            parametersOK = False
        ...
        try:
            numOfNeighbors = int(
                self.gui.frames[coP.ConfigurationsPage].entryNumOfNeighbors.get())
            if numOfNeighbors < 1:
                raise ValueError
        except ValueError:
            parametersOK = False
            self.gui.consolePrint(
                "[ERROR] please check your input for number of neighbors")
        ...
        if len(functions) == 0:
            parametersOK = False
            self.gui.consolePrint(
                "[INFO] please select one or more Haralick functions")
        ...
        if parametersOK:
            ...
            frame = tk.Frame(
                self.gui.frames[fvcP.FeatureVectorCreationPage].middleFrame)
            frame.pack(pady=10)
            ...
            numOfPictures = len(
                os.listdir(
                    self.configuration['processedDataDirectory']))

            progressLabel = tk.Label(
                frame, text="0/" + str(numOfPictures) +
                "   Feature vectors completed.")
            progressLabel.pack(side="left")

            self.gui.frames[fvcP.FeatureVectorCreationPage].progressBars.append(
                progressBar)
            self.gui.frames[fvcP.FeatureVectorCreationPage].progressLabels.append(
                progressLabel)
    ...
    def runConf(self, conf):
        ...
        lbpcm.calculateFeatureVectors(
            self, verbose=True, progressBar=self.gui.frames
            [fvcP.FeatureVectorCreationPage].progressBars[confNumber],
            progressLabel=self.gui.frames[fvcP.FeatureVectorCreationPage].
            progressLabels[confNumber])
    ...
    def classify(self):
        ...
        if self.gui.onlyVotingClassifier.get() == 0:
            ...
            for model in models:
                ...
                output = util.classifyImage(
                    self.pictureToClassify, model, configurations[i])

                self.im[i] = ImageTk.PhotoImage(
                    image=Image.fromarray(
                        util.resizePercent(
                            output, 20)))

                self.gui.frames[clP2.CLP2].pcpFrames[i].labelImage.configure(
                    image=self.im[i])
                i += 1
        ...
        self.gui.frames[clP2.CLP2].labelPeopleCount.configure(
            text="Estimated people count = " +
